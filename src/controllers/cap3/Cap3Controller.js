...
const cap3Domains = function(req,res){
	...
	const file = "./src/public/fileExample.txt";
	fs.readFile(file,charset,domain.intercept((data)=>{
		console.log(data);
		res.render('cap3/index',{...dataJSON, file: {data}});
...
const cap3AsyncParallel = (req,res)=>{
	...
	async.parallel({
		...
		results && Object.values(results).forEach(item=>{
			array.push(item);		
		})
...
module.exports={cap3Index, cap3FileReader, cap3Domains, cap3AsyncSeries, cap3AsyncParallel}